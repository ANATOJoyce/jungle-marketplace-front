...
import { Plus, Package, ChevronLeft, ChevronRight, Trash2, Pencil } from "lucide-react";
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  try {
    const [categoriesRes, productsRes] = await Promise.all([
      fetch(`${process.env.NEST_API_URL}/product/collection/store/${storeId}?page=${page}`, {
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  try {
    if (_action === "delete") {
      ...
      await fetch(`${process.env.NEST_API_URL}/product/collection/${categoryId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      return redirect(`/dashboard/stores/${storeId}/collection`);
    }
    ...
    const res = await fetch(`${process.env.NEST_API_URL}/product/collection/store/${storeId}`, {
      method: "POST",
      headers: {
      ...
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold">Collection du magasin</h1>
              <p className="text-gray-600">{total} collection(s)</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-amber-200 rounded-xl shadow-sm p-6">
          <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">
            <Plus className="w-5 h-5 text-amber-600" /> Créer une nouvelle collection
          </h2>
        ...
        <div className="bg-white rounded-xl border border-amber-100 shadow-sm">
          {categories.length > 0 ? (
              ...
              <table className="w-full">
                <thead className="bg-amber-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left font-semibold text-gray-700 border-b">Handle</th>
                    ...
                    <th className="px-6 py-3 text-left font-semibold text-gray-700 border-b">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-amber-100">
                  {categories.map((c) => (
                    <tr key={c._id} className="hover:bg-amber-50">
                      ...
                      <td className="px-6 py-4 text-gray-600">{c.handle}</td>
            ...
            <div className="text-center py-8 text-gray-600">
              Aucune collection pour cette boutique.
            </div>
          )}
