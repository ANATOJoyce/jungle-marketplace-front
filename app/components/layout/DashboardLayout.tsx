...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  const { token:accessToken, user, isAuthenticated } = useLoaderData<typeof loader>();
  ...
  const navigationItems = [
    ...
    { to: "/dashboard/promotions/index", label: "Promotions", icon: Package },
    { to: "/dashboard/order", label: "Commandes", icon: ShoppingCart },
    { to: "/dashboard/sales-channels/list", label: "Sales Channels", icon: HouseIcon },
  ];
  ...
  useEffect(() => {
    ...
    console.log(token && isAuthenticated, "cond")
            ...
            <div className="flex items-center gap-4">
              ...
              <div>Bienvenu
                <h1 className="text-2xl font-bold text-[#fbb344]"> 
                  {user.first_name || "Tableau de bord"}
                </h1> 
                <p className="text-sm text-gray-500">Gérez votre boutique</p>{store?.name} 
              </div>
            </div>
